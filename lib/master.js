...
class Master extends EventEmitter {
  ...
  /**
   * @constructor  Master构造函数
   * @param {Object} options
   *  - {String} [framework] - specify framework that can be absolute path or npm package
   *  - {String} [baseDir] - 应用的目录，默认是进程的工作目录 `process.cwd()`
   *  - {Object} [plugins] - 定制插件, for unittest
   *  - {Number} [workers] - app workers 的个数, 默认和系统CPU的个数相同 `os.cpus().length`
   *  - {Number} [port] - 默认监听的端口号，默认是7001（http）或者 8443 (https)
